...
class Data_Engineering:
    ...
    def _data_generation(self):
        ...
        if self.conv_method == "max":
            ...
            pool_tool = torch.nn.AvgPool2d((2,2))
        ...
        for i in tqdm(range(len(self.valid_grid)), desc='Processing'):
            ...
            # === Basic Features
            temp_df["fuel_load_cwdc"] = self.fuel_load_cwdc[:,index_value[0],index_value[1]].tolist()
            temp_df["fuel_load_deadcrootc"] = self.fuel_load_deadcrootc[:,index_value[0],index_value[1]].tolist()
            temp_df["fuel_wetness"] = self.fuel_wetness[:,index_value[0],index_value[1]].tolist()
            temp_df["fuel_temperature"] = self.fuel_temperature[:,index_value[0],index_value[1]].tolist()
            temp_df["climate_wind"] = self.climate_wind[:,index_value[0],index_value[1]].tolist()
            temp_df["climate_tbot"] = self.climate_tbot[:,index_value[0],index_value[1]].tolist()
            temp_df["climate_rh2m"] = self.climate_rh2m[:,index_value[0],index_value[1]].tolist()
            temp_df["climate_rain"] = self.climate_rain[:,index_value[0],index_value[1]].tolist()
            temp_df["human_density"] = self.human_density[:,index_value[0],index_value[1]].tolist()
            temp_df["light_frequency"] = self.light_frequency[:,index_value[0],index_value[1]].tolist()
            # temp_df["tree_coverage"] = tree_coverage[:,10,index_value[0],index_value[1]].tolist()

            # === Label
            temp_df["burned_area"] = self.burned_area[:,index_value[0],index_value[1]].tolist()
            ...
            temp_join_list = []
            conv_burned_area = self.burned_area[:,(index_value[0]-1):(index_value[0]+2),(index_value[1]-1):(index_value[1]+2)]
            # judge the shape
            if conv_burned_area.shape[1] == 3 and conv_burned_area.shape[2] == 3:
                for i in range(self.burned_area.shape[0]):
